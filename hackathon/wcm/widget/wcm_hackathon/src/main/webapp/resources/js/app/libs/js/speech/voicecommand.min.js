            ...
            h = h === a ? !0 : !!h, b && b.abort && b.abort(), b = new SpeechRecognition, b.maxAlternatives = 5, b.continuous = !0, b.lang = voice_command_lang || "en-US", b.onstart = function () {
                ...
                for (var b, c = a.results[a.resultIndex], f = 0; f < c.length; f++) {
                    ...
                    for (var g = 0, h = d.length; h > g; g++) {
                        ...
                        let regexp = new RegExp(d[g].originalPhrase, "g");
                        var i = regexp.exec(b);//d[g].command.exec(b);

                        if (i) {
                            var j = i.slice(1);
                            if (d[g].callback) {
                                if (d[g].command.toString().indexOf("#parametro") >= 0) {
                                    var antesDepois = d[g].command.toString().replace("/^", "").replace(/\//g, "").replace(/\\/g, "").replace("$i", "").split("#parametro");
                                    var texto = b;
                                    if(b.indexOf(antesDepois[0])>=0 && antesDepois[0] != "")
                                        texto = b.slice( b.indexOf(antesDepois[0]), b.length).replace(antesDepois[0], "");
                                    if(texto.indexOf(antesDepois[1]) >= 0 && antesDepois[1] != "")
                                        texto = texto.slice(0, texto.indexOf(antesDepois[1])).trim();
                                    d[g].callback.apply(this, [texto])
                                }
                                else
                                    d[g].callback.apply(this, j)
                            }
                            debugState && (root.console.log("command matched: %c" + d[g].originalPhrase, debugStyle), j.length && root.console.log("with parameters", j)), m(e.resultMatch);
                            ...
                            return k.indexOf(d[g].originalPhrase) < 0 && ($("#speech-btn .popover").is(":visible") && $("#speech-btn .popover").fadeOut(250)), !0
                        }
                    }
                }
                
                return m(e.resultNoMatch), $.smallBox({
                    "title": 'Erro: <strong> comando " ' + b + ' " </strong> não encontrado!',
            ...
            for (var e in a)
                if (a.hasOwnProperty(e)) {
                    ...
                    c = l(e), d.push({
                        "command": c,
                        "callback": b,
                        "originalPhrase": e.replace("#parametro", "").trim()
                    })
                }
